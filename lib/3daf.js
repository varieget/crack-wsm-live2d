...
async function loadLive2dWsmAsync(buf) {
  if (buf instanceof ArrayBuffer) {
    return new WsmInstance(await decryptWsmAsync(buf));
  } else {
...
class WsmInstance {
  constructor(buf) {
    ...
    while (i < buf.byteLength) {
      const { total, name, data } = WsmDecoder(buf.slice(i));
...
function WsmDecoder(buf) {
  ...
  let name = new TextDecoder().decode(buf.slice(total, total + i));

  if (name.includes("�")) {
    name = new TextDecoder("gbk").decode(buf.slice(total, total + i));
  }

  if (name.includes("�")) {
    name = new TextDecoder("shift-jis").decode(buf.slice(total, total + i));
  }

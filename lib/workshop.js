...
async function downloadFromWorkshop(url, baseDir) {
    ...
    const file = path.resolve(baseDir, removeInvalidCharsFromFileName(`${title}.wsm`));
    await fs.promises.writeFile(file, inputStream, "binary");
    return file;
}

module.exports = {
    downloadFromWorkshop: downloadFromWorkshop
}